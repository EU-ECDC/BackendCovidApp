...
for(country in c("IT", "CZ", "FR")){
  ...
  list_output <- generate_list_output(country, range_dates, download, data_file, 
                                      total, last_date = TRUE, nsim = nsim, simulations = FALSE)
  
  # Save list_output
  if (!total){
    saveRDS(list_output, paste0("Output/output_model_", country, "_nosim.RDS"))
  } else {
    list_output$pop_age <- list_output$pop * 0
    list_output$pop_age[] <- 1
    saveRDS(list_output, paste0("Output/output_model_", country, "_total_nosim.RDS"))
  }
}
